...
public class SSSPTest extends AbstractBaseTest {
	...
	public static Collection<Object[]> sets() {
		...
		args.addAll(genWorkLoad1(null, null));
	...
	public void testDijkstra3() {
		...
		for (int i = 1; i < NELEMENT; i += OFFSET) {
			// sssp
			// .computeShortestPath(new Node<String>(new Integer(i)
			// .toString()));

			for (int j = 0; j < NELEMENT; j += OFFSET) {
				if (j == i) {
	...
	public void testDijkstra4() {
		...
		graph.addEdge(new Integer(NELEMENT - 1).toString(), new Integer(0)
				.toString(), 1);

		for (int i = 0; i < NELEMENT; i += OFFSET) {
			// sssp
			// .computeShortestPath(new Node<String>(new Integer(i)
			// .toString()));

			for (int j = 0; j < NELEMENT; j += OFFSET) {
				assertEquals((j - i + NELEMENT) % NELEMENT, GraphAlg
