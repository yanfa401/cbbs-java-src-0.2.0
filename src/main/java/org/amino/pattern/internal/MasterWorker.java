 ...
 *
 * @param <S> indicates the type of the input work item.
 * @param <T> indicates the type of the output work item.
 * 
 * @author blainey
 *
 */
public interface MasterWorker<S,T> {
	...
	/**
	 * Submit a work item for processing.  The call is non-blocking.  If, for any reason, the
	 * work item cannot be immediately submitted for execution, then a false result will be
	 * returned.
	 *
	 * @param 	w	Work item.
	 *
	 * @return	Reference to a result key if work item was successfully submitted.
	 * 			null if the work item could not immediately be submitted for processing.
	 *
	 */
	ResultKey submit (S w);

	/**
	 * Submit a work item for processing and block until it is either submitted successfully or
	 ...
	 *
	 * @return	Reference to a result key if work item was successfully submitted.
	 * 			null if the work item could not be submitted, if the time out period expired, or
	 ...
	 * In a static master/worker, this is implied by beginning execution.
	 */
	void finished();

	/**
	 * Begin processing of the work items submitted.  If the master/worker is static,
	 * then further work items may be submitted for processing after execution has started.
	 * The calling thread blocks until all work items have been processed, the specified
	 * timeout value has been reached or if execution was terminated for some abnormal reason.
	 *
	 * In the event of successful execution, all worker threads will be in a completed state prior
	 * to control returning to the calling thread.  In the event of timeout or abnormal
	 * termination, worker threads will be interrupted using the Thread.interrupt() method.
	 *
	 * Upon successful completion, results may be obtained using the result methods.
	 *
	 ...
	 * 			<li>false if the master/worker is not ready to execute (because it hasn't compleed
	 *              a prior execution)
	 *          </ul>
	 *
	 * @throws	TimeoutException
	 * 			If the specified timeout period expired before completion of all work items.
	 *
	 * @throws	InterruptedException
	 * 			If execution of any of the threads was interrupted.
	 *
	 * @throws	ExecutionException
	 * 			If any of the worker threads encounted some runtime exception.
	 */
	boolean execute (long timeout, TimeUnit unit)
	throws TimeoutException, ExecutionException, InterruptedException;

	/**
	 * Begin processing of the work items submitted.  If the master/worker is static,
	 * then further work items may be submitted for processing after execution has started.
	 * Control returns to the calling thread once all tasks have begun execution.
	 * The calling thread may subsequently poll or wait for completion.
	 *
	 * @return	<ul>
	...
	/**
	 * Get ready to begin a new execution of the pattern.
	 * This puts the master/worker back into a state equivalent to just following initial
	 * construction.  It is a faster alternative to creating a new one.
	 *
	 * @return	<ul>
	 * 			<li>true if the master/worker has been restarted
	 * 			<li>false if the master/worker has not yet completed previously submitted work
	 * 			</ul>
	 */
	/*
	boolean restart();
	*/
	 ...
	 *
	 * @param 	k	A result key, obtained from a prior call to "submit"
	 *
	 * @return	The results produced by a worker task, or null if the results are not available.
	 */
	 ...
	 *
	 * @return	The results produced by all worker tasks, or null if one or more workers are
	 *       	still active.
	 */
