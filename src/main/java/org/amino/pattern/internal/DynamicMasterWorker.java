...
/**
 * This is a dynamic master/worker pattern.
 * Like MasterWorker, this patterns allows a master to create workers and submit work for
 * processing.  Unlike MasterWorker, both the master and the workers may submit additional
 * work for processing after execution of workers has begun.
 *
 * The type parameter S indicates the type of the input work item.
 * The type parameter T indicates the type of the output.
 *
...
class DynamicMasterWorker<S,T> extends AbstractMasterWorker<S,T> {
	...
	private class WorkWrapper implements Runnable {
		...
		private WorkItem stealWork() {
			...
			// There are several ways that stealing can happen.
			// In general, work is stolen from the tail of the deque when it is non-empty
			// The order of traversal of deques is important as it can lead to different degrees
			// of cache utilization.  For now, we choose to steal work in a deterministic round-robin
			// manner.  Other options include random deque selection (which may lead to more uniform
			// stealing) or deque selection which is somehow sensitive to actual thread placement on
			// the machine (eg. ideally want to steal from deques for threads which share some level
			// of cache).
		...
		public void run() {
			...
			while (true) {
				...
				if (useLounge) {
					...
					if (debug) System.out.println("Thread " + id + " wait in lounge");

					if (!waitInLounge()) {
						if (debug) System.out.println("Thread " + id + " time to die");
						// Uh oh.  We're out of a job.  Nothing left to live for.
						break;
					}
				}
				*/
				...
				try {
					while(true) {
						//if (debug) System.out.println("Thread " + id + " get work from own queue");

						// Get some work from my queue.
						WorkItem input = workQ[id].poll();

						if (input == null) {
							//if (debug) System.out.println("Thread " + id + " stealWork");

							// Try to steal some work from another queue.
							input = stealWork();

							if (input == null) {
								//if (debug) System.out.println("Thread " + id + " waitForWork");

								// No work available for now.
								// Wait to be notified of new work.
								if (workerPool.waitForWork()) continue;

								if (Thread.interrupted()) return;

								//if (debug) System.out.println("Thread " + id + " no more work for the day");
								// Either no new work or we've been interrupted, so we're done for the day.
								break;
							}
						}

						//if (debug) System.out.println("Thread " + id + " beginning work item");
					...
					//if (debug) System.out.println("Thread " + id + " signalling completion");
		...
		public WorkWrapper (DynamicWorker<S,T> w, int x) {
			...
			this.thisQ = new WorkQueue<S> () {
				...
				public ResultKey submit(S w) {
					//if (debug) System.out.println("Thread " + id + " submit new work");
	...
	private int nextQueue() {
		...
		while (true) {
			old = now.intValue();
			q = (old+1) % numWorkers();
			if (now.weakCompareAndSet(old,q)) break;
		}
		return q;
	}
	...
	public ResultKey submit(S w) {
		...
		// If the work is being submitted by a thread from this master/worker, then
		// lets add it to the associated queue.
		int id = workerPool.threadIndex();
		// Otherwise, we just allocate work round-robin to the available threads.
		if (id < 0) id = nextQueue();

		// Add the work to the appropriate queue.
		ResultKey key = new ResultKeyImpl();
		boolean added = workQ[id].offer(new WorkItem(w,key));

		// The queue is unbounded so should fail to add new entries only in out of memory situations
		assert added;
