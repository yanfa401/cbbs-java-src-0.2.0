...
public class ParallelScanner extends DefaultScanner {
	...
	final private static int MINIMUM_PROBLEM_SIZE=100;

	/**
	 * Creates a new parallel scanner, with the specified number of threads "nthreads".
	 * @param nthreads number of threads to do parallel scan
	 */
	...
	/**
	 * Creates a new parallel scanner, with default number of threads that equals to the the number 
	 * of logical processors.
	 */
	public ParallelScanner() {
		_numberThreads = org.amino.Runtime.getMaxAvailableWorkerThreads();
	}


	/**
	 * Take an array of integer arrays and return a 'flattened' version in a single array.
	 * If it's large enough, parallelize it.
	 * @param input array of integer arrays.  Some elements can be null
	...
	private int[] flatten(final int[][] input){
		...
		if(numberParallelChunks * MINIMUM_PROBLEM_SIZE < totalSize){
			...
			for(int i=0;i < threadPool.length;++i) if(null!=threadPool[i])threadPool[i].start();
			try {for(int i=0;i < threadPool.length;++i) if(null!=threadPool[i])threadPool[i].join();} catch (Exception e){}
		}
	...
	public int[] findAll(final byte[] a, final byte v, int from, int to) {
		...
		if(range < MINIMUM_PROBLEM_SIZE) return super.findAll(a,v,from,to);

		final int numThreads = org.amino.Runtime.reserveThreads(_numberThreads);
		final int[][] tempResults = new int[numThreads][]; // Allocate list of arrays, one per thread
		...
		try {for(int i=0;i < threadPool.length;++i) threadPool[i].join();} catch (Exception e){}
	 ...
	 *  [...,'x','a']['r','a','r','a','a',...]
	 *  are two neighbouring chunks, and we are searching for the string "ara".
	 *  The correct result would point to the string starting just after the element
	 *  'x', but a naive parallel search would think the second element in it's chunk
	 *  was the first location.  To handle this scenario, the thread for each chunk
	 *  searches the preceeding chunk to see if there is a possibility of overlap.  If
	 *  not, then proceed with the naive search.  If there is, then we must determine the
	 *  start of the last string.  For example, if the string arararara is searched for the
	 *  substring ara, then it should see (ara)r(ara)ra, not any other permutation, such as
	 *  ar(ara)r(ara).  In pathalogical cases, e.g. the search string is something like
	 *  arararara... and we search for "ararara", then the method will devolve into serial traversals
	 *  of the preceeding portion of the array for each thread.
	 * @param srcArray source byte array being searched
	 * @param v search byte string
	 * @param endIndex starting index of search backwards.
	 * @return index where a call to findNext would return the next valid search string's index, starting
	 * at endIndex-(v.length-1).
	 */
	private int findPreviousEndIndex(final byte[] srcArray, final byte[] v, int endIndex){
		...
		if(noOverlap) return endIndex;

		int curIndex=searchStart-v.length;
		do {
			searchStart=curIndex+v.length;
			// search forward.  Note it is possible that the start of the search string
			// occurs exactly at the start of the chunk, and the preceeding chunk finished
			// with exactly the substring.  If this is the case, cut the forward search short at
			// endIndex+ v.length-1 and will return -1, so endIndex will be returned.
			 curIndex = serialScanner.findNext(srcArray,v,searchStart,endIndex+v.length-1);
		}while(curIndex >0 && curIndex + v.length < endIndex);
		if(curIndex <0) return endIndex;
	...
	public int findAny(final byte[] a, final byte v, int from, int to) {
		final int range = to - from;
		if(range < MINIMUM_PROBLEM_SIZE) return super.findAny(a,v,from,to);
		...
		for(int i = 0; i < numThreads;++i){
			...
			if(end +chunkSize > to) end = to; // add any remainder to last thread
			final int endIndex=end;
			threadPool[i] = new Thread() {
				public void run() {
					// Periodically check to see if another thread has already found an answer
					...
					for(int j=0;j < numberChunklets;++j){
						if(boxedInt._value >=0){
							break; // another thread successfully found an element
						}
						int chunkletStop = chunkletStart+chunkletSize;
						if(chunkletStop > endIndex) chunkletStop = endIndex;

						int result = serialScanner.findAny(a,v,chunkletStart,chunkletStop);

						chunkletStart = chunkletStop;

						if(result >=0  && boxedInt._value < 0){
							synchronized (boxedInt){
								if(boxedInt._value < 0) boxedInt._value = result;
							}
							break;
						}
					}
	...
	public int findNext(final byte[] a, final byte v, int index) {
		...
		int to = a.length;
		final int range = to - from;
		if(range < MINIMUM_PROBLEM_SIZE) return super.findNext(a,v,index);
		...
		for(int i =0; i < tempResults.length; ++i){
			if(tempResults[i] >=0) return tempResults[i];
		}
		return -1;
	}
	...
	public int findNext(final byte[] a, final byte[] v, int index) {
		...
		for(int i=0;i < threadPool.length;++i) threadPool[i].start();
		for(int i=0;i < threadPool.length;++i) try{threadPool[i].join();} catch(Exception e){}
		org.amino.Runtime.releaseThreads(numThreads);
	...
	public int findPrevious(final byte[] a, final byte v, int index) {
		...
		int to = index;
		final int range = to - from;
		if(range < MINIMUM_PROBLEM_SIZE) return super.findPrevious(a,v,index);
	...
	public int replaceAll(final byte[] a, final byte v, final byte r, int from, int to) {
		final int range = to - from;
		if(range < MINIMUM_PROBLEM_SIZE) return super.replaceAll(a,v,r,from,to);
