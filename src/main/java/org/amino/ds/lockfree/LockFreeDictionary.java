 ...
 * This lock free concurrent dictionary implementation is based on the algorithm
 * defined in the follwoing paper: Scalable and Lock-Free Concurrent
 * Dictionaries By Hakan Sundell and Philippas Tsigas
 *
 * @author raja
 *
 * @param <K> type of key in the dictionary
...
public class LockFreeDictionary<K, V> implements Map<K, V> {
	 ...
	 *
	 * @param <K> type of key in dictionary
	 * @param <V> type of value in dictionary
	 */
	 ...
	 *
	 * @param <V>  type of element in node
	 */
	...
	private int randomLevel() {
		return rdGen.nextInt(MAXLEVEL - 1);
	}
	 ...
	 * @throws ClassCastException k1 should implment comparable interface
	 */
	...
	private Node<K, V> searchLevel(Node<K, V> last, int ll, K k) {
		...
		// if (n1 == head) n1 = n1.next.get(0).getReference();
		stop = null;

		while (true) {
			n2 = n1.next.get(ll).getReference();
			if (n2 == null) {
				if (n1 == last) {
	...
	private V findKey(K k) {
		...
		for (int i = MAXLEVEL - 1; i >= 0; i--) {
			n1 = searchLevel(last, i, k);
			last = n1;
		}
		tn1 = scanKey(last, 0, k);
		last = tn1.n1;
		n2 = tn1.n2;
		val = n2.data.getReference();
	...
	private NodeData<V> insert(K k, V d) {
		...
		int level, i;
		Node<K, V> n1, n2, newN;
		ArrayList<Node<K, V>> sN = new ArrayList<Node<K, V>>();
		NodePair<K, V> tn1;
		V olddata;
		...
		level = randomLevel();
		newN = new Node<K, V>(level, k, d);

		sN.set(MAXLEVEL, head);

		for (i = MAXLEVEL - 1; i >= 0; i--) {
			sN.set(i, searchLevel(sN.get(i + 1), i, k));
		}
		...
		while (true) {
			...
			if (kk == 0) {
				newN.next
						.add(new AtomicMarkableReference<Node<K, V>>(n2, false));
				kk++;
			} else {
				newN.next.set(0, new AtomicMarkableReference<Node<K, V>>(n2,
						false));
			}
		...
		for (i = 1; i < level - 1; i++) {
			...
			while (true) {
				...
				if (kk == 0) {
					newN.next.add(new AtomicMarkableReference<Node<K, V>>(n2,
							false));
					kk++;
				} else {
					newN.next.set(i, new AtomicMarkableReference<Node<K, V>>(
							n2, false));
				}
